...
function normalizeQuestion(raw, variantCode, context) {
  ...
  if (!variantCode) throw new Error(`${context}: variantCode is required.`);
  if (!questionId) throw new Error(`${context}: question id is required.`);
  if (!type) throw new Error(`${context}: question type is required.`);
  if (!text) throw new Error(`${context}: question text is required.`);
  if (!Number.isFinite(points)) throw new Error(`${context}: points must be a valid number.`);
  if (points < 0) throw new Error(`${context}: points must be zero or greater.`);
  if (type === "single_choice" && options.length === 0) throw new Error(`${context}: single_choice questions need at least one option.`);
...
function normalizeVariant(raw, context) {
  ...
  if (!code) throw new Error(`${context}: variant code is required.`);
  if (!level) throw new Error(`${context}: level is required. Use Practicing or Consulting.`);
  if (!EXAM_LEVELS.includes(level)) throw new Error(`${context}: unknown level "${level}". Use Practicing or Consulting.`);
  if (!language) throw new Error(`${context}: language is required.`);
  if (!LANGUAGES.includes(language)) throw new Error(`${context}: unknown language "${language}". Use one of: ${LANGUAGES.join(", ")}.`);
...
function parseTestPackageJson(text) {
  ...
  if (!Array.isArray(parsed.variants)) throw new Error('JSON import must include a "variants" array.');
  if (!parsed.questions || typeof parsed.questions !== "object" || Array.isArray(parsed.questions)) throw new Error('JSON import must include a "questions" object keyed by variant code.');
  ...
  const unknownQuestionGroups = Object.keys(parsed.questions).filter((code) => !variantCodes.has(code));
  if (unknownQuestionGroups.length) throw new Error(`JSON questions reference unknown variantCode: ${unknownQuestionGroups.join(", ")}.`);
  const questions = {};
  ...
  variants.forEach((variant) => {
    ...
    if (rows.length === 0) throw new Error(`${variant.code}: questions array is empty.`);
...
function parseTestPackageCsv(text) {
  ...
  const required = ["variantCode", "level", "language", "questionId", "type", "points", "text", "optionA", "optionB", "optionC", "optionD", "correctAnswer"];
  required.forEach((column) => {
    if (!(column in index)) throw new Error(`CSV is missing required column "${column}". Check the header row.`);
  });
...
const sampleTestPackage = {
  variants: [
    { code: "PRACTICING_SAMPLE_EN", level: "Practicing", language: "EN", title: "Practicing sample EN", status: "Approved" },
    { code: "CONSULTING_SAMPLE_EN", level: "Consulting", language: "EN", title: "Consulting sample EN", status: "Approved" },
  ],
  questions: {
    PRACTICING_SAMPLE_EN: [
      {
        questionId: "P-SAMPLE-01",
        type: "single_choice",
        points: 1,
        text: "Which document confirms the Candidate identity at the start of the exam?",
        options: ["Passport or ID card", "Tree inspection form", "Weather report", "Site map"],
        correctAnswer: "Passport or ID card",
      },
      {
        questionId: "P-SAMPLE-02",
        type: "text",
        points: 3,
        text: "Describe one observable defect that should be recorded during an outdoor tree inspection.",
        options: [],
        correctAnswer: "Any relevant observable defect with a clear description.",
      },
    ],
    CONSULTING_SAMPLE_EN: [
      {
        questionId: "C-SAMPLE-01",
        type: "single_choice",
        points: 1,
        text: "For a Consulting report, what should the Candidate document for each selected tree?",
        options: ["Observed condition and management reasoning", "Only species name", "Only final score", "Only GPS coordinates"],
        correctAnswer: "Observed condition and management reasoning",
      },
      {
        questionId: "C-SAMPLE-02",
        type: "text",
        points: 4,
        text: "Summarise the likely management implications for a mature tree with visible structural defects.",
        options: [],
        correctAnswer: "A proportionate response considering risk, value, inspection evidence and management options.",
      },
    ],
  },
};
...
function sampleTestPackageCsv() {
  const headers = ["variantCode", "level", "language", "questionId", "type", "points", "text", "optionA", "optionB", "optionC", "optionD", "correctAnswer"];
  const rows = sampleTestPackage.variants.flatMap((variant) => (
    sampleTestPackage.questions[variant.code].map((question) => {
...
function downloadSampleCsv() {
  downloadTextFile("vetbara-test-package-sample.csv", sampleTestPackageCsv(), "text/csv;charset=utf-8");
}

function downloadSampleJson() {
  downloadTextFile("vetbara-test-package-sample.json", JSON.stringify(sampleTestPackage, null, 2), "application/json;charset=utf-8");
}
...
export default function VetBaraPrototype() {
  ...
  async function sendSyncEvent(event) {
    ...
    try {
      ...
      setSync((prev) => prev.map((item) => item.id === syncId ? { ...item, status: "Sync error" } : item));
    }
  }
  ...
  async function loadEvaluationPreview(candidateId) {
    ...
    try {
      ...
      setEvaluationError("Evaluation preview is not available right now.");
    } finally {
  ...
  async function downloadDraftExport(candidateId) {
    ...
    try {
      ...
      setExportError("Draft export is not available right now.");
    } finally {
  ...
  async function handleLoadCentreSetup() {
    if (!activeSessionToken) {
      setCentreSetupError("Open the Centre portal with a valid QR/session before loading or saving setup.");
      return;
    }
  ...
  async function handleDownloadCentreAuditPackage() {
    ...
    try {
      ...
      setCentreAuditExportError("Audit Package could not be downloaded. Check the Centre session and try again.");
    } finally {
  ...
  return <main className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      {role === "Centre" && <CentreView centreUnlocked={centreUnlocked} centreCode={centreCode} setCentreCode={setCentreCode} unlockCentre={unlockCentre} enabledLevels={enabledLevels} toggleLevel={toggleLevel} language={language} availableVariants={availableVariants} variants={variants} setVariants={setVariants} importTestPackage={importTestPackage} testImportStatus={testImportStatus} testImportError={testImportError} testImportSummary={testImportSummary} candidates={candidates} selectedCandidateId={selectedCandidateId} setSelectedCandidateId={setSelectedCandidateId} addCandidate={addCandidate} updateCandidate={updateCandidate} assignments={assignments} setAssignments={setAssignments} examiners={examiners} candidateQrFor={(id) => payload("Candidate", id)} examinerQrFor={(id) => payload("Examiner", id)} centreSetupLoading={centreSetupLoading} centreSetupSaving={centreSetupSaving} centreSetupError={centreSetupError} centreSetupStatus={centreSetupStatus} centreAuditExportLoading={centreAuditExportLoading} centreAuditExportError={centreAuditExportError} centreQrAccess={centreQrAccess} centreValidationIssues={centreValidationIssues} candidateConfirmed={candidateConfirmed} candidateStatus={candidateStatus} candidateTimes={candidateTimes} testResponses={testResponses} reportDrafts={reportDrafts} outdoor={outdoor} handleLoadCentreSetup={handleLoadCentreSetup} handleSaveCentreSetup={handleSaveCentreSetup} handleDownloadCentreAuditPackage={handleDownloadCentreAuditPackage} updateExaminer={updateExaminer} addExaminer={addExaminer} />}
...
function PilotWorkflowDashboard({ candidates, assignments, examiners, centreValidationIssues, testImportSummary, candidateConfirmed, candidateStatus, candidateTimes, testResponses, reportDrafts, outdoor, centreSetupStatus }) {
