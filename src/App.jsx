...
export default function VetBaraPrototype() {
  ...
  async function loadEvaluationPreview(candidateId) {
    if (!activeSessionToken) {
      setEvaluationError("Evaluation preview requires backend session.");
      return;
    }
  ...
  async function downloadDraftExport(candidateId) {
    if (!activeSessionToken) {
      setExportError("Draft export requires backend session.");
      return;
    }
  ...
  async function handleLoadCentreSetup() {
    ...
    try {
      ...
      setCentreSetupError("Centre setup is not available right now.");
    } finally {
  ...
  async function handleSaveCentreSetup() {
    ...
    try {
      ...
      setCentreSetupError("Centre setup could not be saved right now.");
    } finally {
  ...
  return <main className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      {role === "Centre" && <CentreView centreUnlocked={centreUnlocked} centreCode={centreCode} setCentreCode={setCentreCode} unlockCentre={unlockCentre} enabledLevels={enabledLevels} toggleLevel={toggleLevel} language={language} availableVariants={availableVariants} variants={variants} setVariants={setVariants} importTestPackage={importTestPackage} candidates={candidates} selectedCandidateId={selectedCandidateId} setSelectedCandidateId={setSelectedCandidateId} addCandidate={addCandidate} updateCandidate={updateCandidate} assignments={assignments} setAssignments={setAssignments} examiners={examiners} candidateQrFor={(id) => payload("Candidate", id)} examinerQrFor={(id) => payload("Examiner", id)} centreSetupLoading={centreSetupLoading} centreSetupSaving={centreSetupSaving} centreSetupError={centreSetupError} centreSetupStatus={centreSetupStatus} centreQrAccess={centreQrAccess} centreValidationIssues={centreValidationIssues} handleLoadCentreSetup={handleLoadCentreSetup} handleSaveCentreSetup={handleSaveCentreSetup} updateExaminer={updateExaminer} addExaminer={addExaminer} />}
